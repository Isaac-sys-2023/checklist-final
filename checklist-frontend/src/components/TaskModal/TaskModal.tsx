...
const TaskModal = ({ onClose, task }: TaskModalProps) => {
    const [taskTitle, setTaskTitle] = useState(task?.title || "");
    const [taskDate, setTaskDate] = useState(task?.day || new Date().toISOString().split("T")[0]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (taskTitle.trim()) {
            if (id !== null) {
                try {
                    const response = await fetch(`${API_URL}/tasks/${id}/entire`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({ title: taskTitle, completed: taskCompleted, day: taskDate }),
                    });
                    if (!response.ok) {
    ...
    return(
        <div style={{padding: '1rem', background: '#444', borderRadius: '8px'}}>
            <h1 style={{padding: '0 1rem 1rem 1rem', fontSize: '1.5rem',textAlign: 'center'}}>{task ? "Update Task" : "New Task"}</h1>
